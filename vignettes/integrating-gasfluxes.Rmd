...
As noted in the [introductory vignette](intro-to-whattheflux.html), there are
...
The `whattheflux` package provides extensive outputs based on linear
...
# Load the data
# Data from a LI-7810
f <- system.file("extdata/TG10-01087-curvature.data", package = "whattheflux")
dat <- wtf_read_LI7810(f)

# Fit a linear flux and QA/QC it
flux <- wtf_compute_fluxes(dat, group_column = NULL, time_column = "TIMESTAMP", conc_column = "CO2")
print(flux$slope_estimate)
print(flux$adj.r.squared)
print(flux$r.squared_poly)
```

There's a fairly large jump from the R<sup>2</sup> of the linear
model (0.93) to that of a polynomial (0.99+). Visualizing the data:

```{r qaqc-plot, fig.width=6}
library(ggplot2)
dat$SECONDS <- dat$SECONDS-min(dat$SECONDS)
ggplot(dat, aes(SECONDS, CO2)) + geom_point() + geom_smooth(method = "lm")
```
...
computed based on only the first ~75 seconds, which looks linear.

A second option would be fit more sophisticated model(s).
...
parameter, but we can also use use `whattheflux::wtf_compute_fluxes()` to
...
wtf_compute_fluxes(dat, NULL, "SECONDS", "CO2", fit_function = f)
```

## Conclusion

This vignette covered how to integrate `gasfluxes` with `whattheflux`, if
